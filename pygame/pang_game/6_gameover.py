...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_idx #해당무기없애기 위한 값 설정
                ball_to_remove = ball_idx # 해당 공 없애기 위한 값 설정
                # 가장 작은 공의 크기가 아니라면 다음단계의 크기의 공으로 간다.
                if ball_img_idx < 3:
                    #현재 공 크기 정보를 가지고 온다.
                ...
                continue # 안쪽 for 문 조건 맞지 않을시 continue 바깥 for 문 계속 수행
            break # 안쪽 for break 를 만나면 여기로 진입 2중 for 문 한번에 탈출
    # for 바깥조건:
    #     바깥동작
    #     for 안쪽조건:
    #         안쪽동작
    ...
    #             break
    #     else: 
    #         continue 안쪽 조건 맞지않을시 바깥 포문 계속 실행
    #     break 여기서 2중포문 둘다 탈출 (안쪽에서 break 당할 시)
